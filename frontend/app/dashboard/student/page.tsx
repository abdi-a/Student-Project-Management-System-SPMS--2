...
export default function StudentDashboard() {
    ...
    const MOCK_TASKS = [
        { id: 1, title: 'Submit Proposal', status: project ? 'Completed' : 'Pending', deadline: 'N/A' },
    ];
                    ...
                    {project ? (
                        <div>
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500 mt-1">
                                        Supervisor: {project.supervisor ? project.supervisor.name : 'Not Assigned'}
                                    </p>
                                </div>
                            ...
                            <div className="w-full bg-gray-200 rounded-full h-2.5 mb-2">
                                <div className="bg-blue-600 h-2.5 rounded-full" style={{ width: project.status === 'completed' ? '100%' : '50%' }}></div>
                            </div>
                            <span className="text-xs text-gray-500">{project.status === 'completed' ? '100%' : 'In Progress'}</span>
                        </div>
                    ) : (
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between p-3 bg-red-50 rounded border-l-4 border-red-500">
                                <div>
                                    ...
                                    <p className="text-xs text-red-600">TBD</p>
                                </div>
                            </div>
                        </div>
                    )}
